...
const ClientNavbar = () => {
  ...
  useEffect(() => {
    const fetchClientDetails = async () => {
      try {
        ...
        setClientName(response.data[0]?.clientName || ''); // Set client name from response
  ...
  const handleProfileClick = () => {
    navigate("/clientProfileView"); // Modify this path as needed
      ...
      >
        ...
        <ul className="mt-8 ml-4">
          <li className="flex items-center p-2 text-white hover:bg-[#3a386f] transition">
            <a href="/clientdashboard" className="flex items-center">
              <FontAwesomeIcon
